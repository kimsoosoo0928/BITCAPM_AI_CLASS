...
# hidden layer 01
w1 = tf.Variable(tf.random_normal([2,3]), name='weight')
b1 = tf.Variable(tf.random_normal([3]), name='bias')

layer1 = tf.sigmoid(tf.matmul(x, w1) + b1)

# hidden layer 02
w2 = tf.Variable(tf.random_normal([3,3]), name='weight')
b2 = tf.Variable(tf.random_normal([3]), name='bias')

layer2 = tf.sigmoid(tf.matmul(layer1, w2) + b2)

# output layer
w3 = tf.Variable(tf.random_normal([3,1]), name='weight')
